...
class Model:
    ...
    def __setup(self):
        ...
        for i in range(len(self.__numHiddenLayerNodes) + 1): # For every hidden layer + the output layer...
            ...
            weight = _model_helper._weight_initialization(currLayerNodesNum, prevLayerNodesNum, self.__numInputNodes, self.__numOutputNodes, self.__weightInitFunc[i] if isinstance(self.__weightInitFunc, list) else self.__weightInitFunc)
            bias = _model_helper._bias_initialization(currLayerNodesNum, self.__biasInitFunc[i] if isinstance(self.__biasInitFunc, list) else self.__biasInitFunc)
    ...
    def __feed_forward(self, input):
        ...
        for i in range(len(self.__numHiddenLayerNodes) + 1): # For each layer in the neural network...
            ...
            # current layer.
            currLayer = _model_helper._activation(currLayer, self.__activationFunc[i] if isinstance(self.__activationFunc, list) else self.__activationFunc)
